...
class StochGenMatrixSplittingTest : public StochGenMatrix, public ::testing::TestWithParam<std::tuple<std::vector<unsigned int>,std::vector<int>,std::vector<unsigned int>>>
{
   public:
      StochGenMatrix* createTestMatrix( unsigned int n_link_vars, unsigned int m_link_vars,
            unsigned int n_diag, unsigned int m_diag, unsigned int n_blocks, unsigned int m_link_cons ) const;
};

StochGenMatrix* StochGenMatrixSplittingTest::createTestMatrix(unsigned int n_link_vars, unsigned int m_link_vars,
            unsigned int n_diag, unsigned int m_diag, unsigned int n_blocks, unsigned int m_link_cons ) const
{
...
INSTANTIATE_TEST_CASE_P(
      ...
      StochGenMatrixSplittingTest,
      ::testing::Values(
            std::make_tuple(
                  std::vector<unsigned int>{5,5,10,3,10,25},
                  std::vector<int>{2,2,2,2,2,2,2,2,2,0},
                  std::vector<unsigned int>{0,0,0,1,1,2,2,2,2,3}
            ),
            std::make_tuple(
                  std::vector<unsigned int>{10,2,2,2,10,18},
                  std::vector<int>{2,2,2,2,2,2,2,2,2,0},
                  std::vector<unsigned int>{0,0,0,1,1,2,2,2,2,3}
            ),
            std::make_tuple(
                  std::vector<unsigned int>{4,6,1,5,10,25},
                  std::vector<int>{2,1,0,0,3,2,2,2,2,0},
                  std::vector<unsigned int>{0,0,0,1,1,2,2,2,3,3}
            ),
            std::make_tuple(
                  std::vector<unsigned int>{5,5,10,3,10,25},
                  std::vector<int>{2,3,4,2,2,1,0,3,3,0},

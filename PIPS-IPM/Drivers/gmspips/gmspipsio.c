      ...
      const char* GAMSSysDir)   /** < GAMS system directory to locate shared libraries (can be NULL) */
{
   ...
   if ((equEl && equIl) || (equEm && equIm)) {
      for (; symStart <= symCnt; symStart++) {
         ...
         while (gdxDataReadRaw(fDCT, keyInt, vals, &dimFirst)) {
            if (gequl)
               wvals[GMS_VAL_LEVEL] = gequl[rowsSeen];
            if (gequm)
               wvals[GMS_VAL_MARGINAL] = gequm[rowsSeen];
            rowsSeen++;
   ...
   if (varl || varm) {
      for (; symStart <= symCnt; symStart++) {
         ...
         while (gdxDataReadRaw(fDCT, keyInt, vals, &dimFirst)) {
            if (gvarl)
               wvals[GMS_VAL_LEVEL] = gvarl[colsSeen];
            if (gvarm)
               wvals[GMS_VAL_MARGINAL] = gvarm[colsSeen];
            colsSeen++;
            GDXSAVECALLX(fSOL, gdxDataWriteRaw(fSOL, keyInt, wvals));
         }
         GDXSAVECALLX(fDCT, gdxDataReadDone(fDCT));
...
#if !defined(GDXSOURCE)
int
doColumnPermutation(const gmoHandle_t gmo, const int strict, const int n, const int stageI, const int stage0, int32_t* n0, int32_t* ni, int perm[]) {
   ...
   if (rc) PRINTANDEXIT("Problems accessing variable stages");
   ...
   for (j = 0; j < n; j++) {
      ...
      else if (strict) PRINTANDEXIT("Unmatched stage %d of column %s while reading stage %d", (int) stages[j], gmoGetVarNameOne(gmo, j, colname),
            stageI);
   }
   *ni = *n0;
   *n0 = 0;
...
int
doRowPermutation(const gmoHandle_t gmo, const int strict, const int m, const int stageI, const int stageN, int32_t* meq, int32_t* mleq, int32_t* mLeq,
      int32_t* mLleq, int perm[]) {
   ...
   if (rc) PRINTANDEXIT("Problems accessing equation stages");
   rc = gmoGetEquType(gmo, eTypes);
   if (rc) PRINTANDEXIT("Problems accessing equation types");

   for (i = 0; i < m; i++) {
      if (gmoequ_N == eTypes[i])
         continue;
      if (eTypes[i] > gmoequ_N) PRINTANDEXIT("Unknown row type %d in row %s", eTypes[i], gmoGetEquNameOne(gmo, i, rowname));
      ...
      else if (strict) PRINTANDEXIT("Unmatched stage %d of row %s while reading stage %d of %d stages", (int) stages[i],
            gmoGetEquNameOne(gmo, i, rowname), stageI, stageN);
   }
   *mLleq = *meq + *mleq + *mLeq;
   *mLeq = *meq + *mleq;
   *mleq = *meq;
...
int fillMatrix(const gmoHandle_t gmo, const int mStart, const int mEnd, int32_t rm[], int col[], double val[], int64_t* nnz) {
   ...
   rm[0] = 0;
   for (i = 0; i < m; i++) {
      if (gmoGetRowSparse(gmo, i + mStart, colidx, jacval, NULL, &nz, &nlnz)) PRINTANDEXIT("Cannot get row %d in sparse format", i + mStart);
      assert(0 == nlnz);
      rm[i + 1] = rm[i] + nz;
      ...
      GMSPIPSBlockData_t* blk)       /** < block structure to be filled */
{
   ...
   if (GAMSSysDir) {
      ...
      if (!rc) PRINTANDEXIT("Could not create gev object: %s\n", msg);
      rc = gmoCreateD(&fGMO, GAMSSysDir, msg, sizeof(msg));
      if (!rc) PRINTANDEXIT("Could not create gmo object: %s\n", msg);
   }
   else {
   ...
   if (rc) PRINTANDEXIT("Failed gevInitEnvironmentLegacy\n");

   rc = gmoRegisterEnvironment(fGMO, fGEV, msg);
   if (rc) PRINTANDEXIT("Could not register gev environment: %s\n", msg);

   rc = gmoLoadDataLegacy(fGMO, msg);
   if (rc) PRINTANDEXIT("Could not load data: %s\n", msg);

   /* Make sure objective variable and objective row are at the end */
   gmoIndexBaseSet(fGMO, 0);
   if (gmoN(fGMO) != gmoObjVar(fGMO) - 1) PRINTANDEXIT("Objective variable not at the of columns\n");

   if (gmoM(fGMO) != gmoObjRow(fGMO) - 1) PRINTANDEXIT("Objective equation not at the of rows\n");
   ...
   if (0 == actBlock) {
      ...
      if (rc) PRINTANDEXIT("Could not install row permutation\n");
      free(rowPerm);

      rc = gmoSetRvVarPermutation(fGMO, colPerm, pipsn);
      if (rc) PRINTANDEXIT("Could not install column permutation\n");
      free(colPerm);

      /* Now fill the block */

      mAStart = 0;
      mAEnd = mAStart + blk->mA;
      mCStart = mAEnd;
      mCEnd = mCStart + blk->mC;
      mBLStart = mCEnd;
      mBLEnd = mBLStart + blk->mBL;
      mDLStart = mBLEnd;
      mDLEnd = mDLStart + blk->mDL;
      ...
      if (!gmoGetObjVector(fGMO, blk->c, NULL)) PRINTANDEXIT("Could not get objective vector\n");
      if (!gmoGetVarLower(fGMO, blk->xlow)) PRINTANDEXIT("Could not get variable lower bound\n");
      if (!gmoGetVarUpper(fGMO, blk->xupp)) PRINTANDEXIT("Could not get variable lower bound\n");
...

      {
         ...
         if (!gmoGetRhs(fGMO, rhs)) PRINTANDEXIT("Could not get rhs vector\n");
         if (!gmoGetEquType(fGMO, eType)) PRINTANDEXIT("Could not get equation type vector\n");
...
void copyGDXSymbol(int numBlocks, int actBlock, gdxHandle_t bGDX[], gdxHandle_t fGDX, const char* symName, const int nUelOffSet, const int offSet,
      const int stage[], const int vstage[], const int estage[], const int linkingBlock, const int readType, const int objVarUel,
      const int objRowUel) {
   int k, rc;
      ...
      GMSPIPSBlockData_t* blk)   /** < block structure to be filled */
{
   ...
   while (gdxDataReadRaw(fGDX, keyInt, vals, &dimFirst)) {
      ...
      if (!(varPerm[col] && equTypeNr[row])) {
         ...
         if (debugMode)
            printf(
                  "*** Unexpected matrix coefficient %f of equation %s (stage=%d) and variable %s (stage=%d) while scanning for block index %d [col %d coluel %d vp %d row %d rowuel %d et %d]\n",
                  vals[GMS_VAL_LEVEL], rowname[row], rowstage[row], varname[col], varstage[col], actBlock + offset, col, keyInt[1], varPerm[col], row,
                  keyInt[0], equTypeNr[row]);
      }
   ...
   if (badCnt)
      printf("*** %d unexpected matrix coefficient var.stage <> equ.stage and not linking. %s\n", badCnt,
            debugMode ? "" : "Add -d to gmschk call to see details.");
   assert(0 == badCnt);

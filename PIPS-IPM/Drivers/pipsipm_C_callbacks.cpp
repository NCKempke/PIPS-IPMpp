...
extern "C" void
PIPSSolve(MPI_Comm comm, void* user_data, int numScens, int nx0, int my0, int mz0, int nx, int my, int mz, FMAT fQ, FNNZ fnnzQ, FVEC fc, FMAT fA,
      FNNZ fnnzA, FMAT fB, FNNZ fnnzB, FVEC fb, FMAT fC, FNNZ fnnzC, FMAT fD, FNNZ fnnzD, FVEC fclow, FVEC ficlow, FVEC fcupp, FVEC ficupp,
      FVEC fxlow, FVEC fixlow, FVEC fxupp, FVEC fixupp, double* obj_val, double* first_primal, double* second_primal, double* first_dual,
      double* second_dual) {
   ...
   pipsIpm.go();
   if (mype == 0)
      cout << "solving ..solving done." << endl;

   obj_val[0] = pipsIpm.getObjective();
   for (int i = 0; i < nx0; i++) {
      first_primal[i] = pipsIpm.getFirstStagePrimalColSolution()[i];
   }
   for (int i = 0; i < my + mz0; i++) {
      first_dual[i] = pipsIpm.getFirstStageDualRowSolution()[i];
   }
   int cnt1 = 0;
   // int cnt2 = 0;
   for (int i = 0; i < numScens; i++) {

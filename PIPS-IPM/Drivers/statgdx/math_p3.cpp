...
Function(SYSTEM_double) MATH_P3_arccos(SYSTEM_double x) {
...
Function(SYSTEM_double) MATH_P3_arcsin(SYSTEM_double x) {
   SYSTEM_double result;
...
Function(SYSTEM_double) MATH_P3_arctan2(SYSTEM_double y, SYSTEM_double x) {
   SYSTEM_double result;
...
Function(SYSTEM_double) MATH_P3_tan(SYSTEM_double x) {
   SYSTEM_double result;
...
Function(SYSTEM_double) MATH_P3_intpower(SYSTEM_double x, SYSTEM_integer i) {
   SYSTEM_double result;
   SYSTEM_integer y;
...
Function(SYSTEM_double) MATH_P3_power(SYSTEM_double x, SYSTEM_double y) {
   SYSTEM_double result;
   ...
   else if (SYSTEM_frac(y) == 0.0 && SYSTEM_abs_r(y) <= SYSTEM_maxint) {
...
Function(SYSTEM_double) MATH_P3_roundto(SYSTEM_double x, SYSTEM_integer i) {
   SYSTEM_double result;
   typedef SYSTEM_uint8 _sub_0ROUNDTO;
   typedef SYSTEM_double tfactors[2];
   typedef tfactors _arr_1ROUNDTO[41];
   static _arr_1ROUNDTO lfactorarray = {{1e-20, 1e20},
                                        {1e-19, 1e19},
                                        {1e-18, 1e18},
                                        {1e-17, 1e17},
                                        {1e-16, 1e16},
   ...
   if (i < -20 || i > 20)
      _P3_RAISE(ValueCast(EXCEPTIONS_einvalidop, SYSTEM_exception_DOT_create(ValueCast(SYSTEM_exception, _P3alloc_object(&EXCEPTIONS_einvalidop_CD)),
            _P3str1("\035RoundTo arg 2 is out of range"))));
...
Function(SYSTEM_boolean) MATH_P3_isnan(SYSTEM_double avalue) {
...
Function(SYSTEM_boolean) MATH_P3_isinfinite(SYSTEM_double avalue) {
   SYSTEM_boolean result;
   MATH_P3_ti64rec i64rec;
   SYSTEM_int64 mantissa;
...
Function(SYSTEM_double) MATH_P3_lnxp1(SYSTEM_double x) {
   SYSTEM_double result;
...
Function(_P3set_elem *) MATH_P3_getexceptionmask(_P3set_elem* result, SYSTEM_uint8 _len_ret) {
...
Function(_P3set_elem *) MATH_P3_setexceptionmask(_P3set_elem* result, SYSTEM_uint8 _len_ret, const _P3set_elem* mask) {
   _P3SET_copy(result, _len_ret, _P3empty_set);
...
# endif
...
#elif defined(__linux__)
   {
      ...
      if (cw & FE_INVALID)
         ADD2MASK(EX_INVALIDOP);
      if (cw & FE_DENORMAL)
         ADD2MASK(EX_DENORMAL);
      if (cw & FE_DIVBYZERO)
         ADD2MASK(EX_ZERODIVIDE);
      if (cw & FE_OVERFLOW)
         ADD2MASK(EX_OVERFLOW);
      if (cw & FE_UNDERFLOW)
         ADD2MASK(EX_UNDERFLOW);
      if (cw & FE_INEXACT)
         ADD2MASK(EX_PRECISION);
      newcw = 0;
      if (ISINMASK(MATH_P3_exinvalidop))
         newcw |= FE_INVALID;
      if (ISINMASK(MATH_P3_exdenormalized))
         newcw |= FE_DENORMAL;
      if (ISINMASK(MATH_P3_exzerodivide))
         newcw |= FE_DIVBYZERO;
      if (ISINMASK(MATH_P3_exoverflow))
         newcw |= FE_OVERFLOW;
      if (ISINMASK(MATH_P3_exunderflow))
         newcw |= FE_UNDERFLOW;
      if (ISINMASK(MATH_P3_exprecision))
         newcw |= FE_INEXACT;

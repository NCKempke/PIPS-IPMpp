...
int main(int argc, char** argv) {
   ...
   int myBatchPe;
   MPI_Comm_rank(commBatch, &myBatchPe);
   ...
   {
      stringstream ss;
      ss << datadirname << (color + 1) << "/mean-prob/" << datarootnameMean;
   ...
   if (myBatchPe == 0) {
      {
         ...
         file1stStg << scientific;
         file1stStg.precision(16);
   ...
   for (int scen = 0; scen < nscen; scen++) {
      if (scen % nbatchprocs == myBatchPe) {
         ...
         {
            stringstream ss;
            ss << outputdir << "/batch-" << (1 + color) << "-out_primals_recou" << (scen + 1) << ".txt";
         ...
         {
            stringstream ss;
            ss << outputdir << "/batch-" << (1 + color) << "-out_duals_recou" << (scen + 1) << ".txt";

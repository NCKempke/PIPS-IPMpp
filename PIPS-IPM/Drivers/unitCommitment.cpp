...
public: //methods
   ...
   void loadWindScenario(int scenario, int tmStep, double* data, int noWindUnits);
#ifdef TIMING
...
int main(int argc, char* argv[]) {
   ...
   while (iargv < argc) {
      ...
      if (0 == strcmp("solve", &argv[iargv][iopt])) {
         ...
         if ('/' != g_strDataDir[g_strDataDir.length() - 1])
            g_strDataDir += "/";
#ifdef TIMING
   ...
   if (argc > 1 && iargv == argc && goodUsage) {
      ...
      if (solveWith != 0 && solveWith != 1 && solveWith != 2 && solveWith != 3 && solveWith != 4 && solveWith != 5 && solveWith != 6 &&
          solveWith != 11 && solveWith != 12 && solveWith != 13 && solveWith != 20) {
...
void stochSolve(int argc, char* argv[], PbData& pbData, int printx, int solveWith) {
   ...
   if (myRank == 0) {
    ...
    printf("L2 VAR %4d EQ %4d INEQ %4d\n", n, my, mz);
      ...
      printf("N=%d  T=%d S=%d  ND=%d  NW=%d NWT=%d\n", pbData.N, pbData.T, pbData.S, pbData.ND, pbData.NW, pbData.NWT);
...
extern "C" int fnnzQ(void* user_data, int id, int* nnz) { *nnz = 0; }
   ...
   if (id == 0) {
      ...
      for (int t = 1; t <= ND; t++) {
         ...
         for (int k = 0; k < t + 1; k++) {
            ...
            for (int line = 0; line < N; line++) {
               ...
               jcolM[colind] = N * T + k * N + line;
               assert(N * T + k * N + line < 2 * N * T);
               M[colind] = 1.0;
               colind++;
    ...
    assert(colind==2*N*(T-1));
      ...
      for (int j = 0; j < N; j++) {
         ...
         colind++; //v[j,1]
         jcolM[colind] = N + j;
         M[colind] = SU[j] - Pmax[j];
         colind++; //v[j,2]
         //jcolM[colind] = 3*N*T+j; M[colind] = 1.0;           colind++;
         jcolM[colind] = 3 * N * T + j;
         M[colind] = 1.0;
         colind++; //pc[j,1,1]
         krowM[rowind + 1] = colind;
         rowind++;
    ...
    assert(colind==(N+NW)*(T-1) + (N+NW)*(T-1) + N*(T-1) + 2*N*(T-1) + N*(T-1));
      ...
      for (int k = 1; k < T - 1; k++) {
         for (int j = 0; j < N; j++) {
            ...
            colind++;
            //C-> -identity
            jcolM[colind] = N * (T - 1) + NW * (T - 1) + k * N + j;
            M[colind] = -1.0;
            colind++; //!2
            krowM[rowind + 1] = colind;
            rowind++;
...
extern "C" int fb(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
      ...
      for (int j = 0; j < N; j++)
         if (G[j] > 0 || L[j] > 0)
            cerr << " feature not implemented (16 & 19 blocks)" << endl;
...
extern "C" int fclow(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
      ...
      for (int j = 0; j < N; j++) {
         ...
         for (int k = G[j] + 1; k < T - UT[j] + 1; k++)
            vec[it++] = 0.0;
      }
      // ------ 18 ------
      for (int j = 0; j < N; j++) {
         for (int k = T - UT[j] + 2; k <= T; k++)
            vec[it++] = 0.0;
      }
      // ------ 20 ------
      int* DT = data->DT;
      ...
      for (int j = 0; j < N; j++) {
         ...
         for (int k = L[j] + 1; k < T - DT[j] + 1; k++)
            vec[it++] = -DT[j];
      }
      // ------ 21 ------
      for (int j = 0; j < N; j++) {
         for (int k = T - DT[j] + 2; k <= T; k++)
            vec[it++] = -1.0 - T + k;
...
extern "C" int fxlow(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
      ...
      for (int i = 0; i < len; i++)
         vec[i] = 0.0; //no limit or 0 as lower limit
...
extern "C" int fixlow(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
      for (int i = 0; i < NT; i++)
         vec[i] = 1.0; //lower limit on v
      for (int i = NT; i < 3 * NT; i++)
         vec[i] = 1.0; //lower limit on cu and cd;
      for (int i = 3 * NT; i < 3 * NT + N; i++)
         vec[i] = 1.0; //lower limit on pc[j,1]

      for (int i = 3 * NT + N; i < 3 * NT + 2 * N; i++)
         vec[i] = 1.0; //lower limit on pcmax[j,1]
      for (int i = 3 * NT + 2 * N; i < 3 * NT + 2 * N + NW; i++)
         vec[i] = 1.0; //lower limit on pwind[j,1,1]
   }
   else {
      int NTm1 = N * (T - 1);
      assert(len == 2 * NTm1 + NW * (T - 1));
      for (int i = 0; i < NTm1; i++)
         vec[i] = 1.0; //low limit on pc
      //for(int i=NTm1; i<2*NTm1; i++) vec[i]=0.0; //no low limit cp //!2
      for (int i = NTm1; i < NTm1 + NW * (T - 1); i++)
         vec[i] = 1.0; //low on pcwind
      for (int i = NTm1 + NW * (T - 1); i < 2 * NTm1 + NW * (T - 1); i++)
         vec[i] = 1.0; //low on pcmax
      assert(len == 2 * NTm1 + NW * (T - 1));
   }
   return 0;
}
...
extern "C" int fxupp(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
...
#endif
      for (int i = 0; i < N * T; i++)
         vec[i] = 1.0; //upper on v
      for (int i = N * T; i < 3 * NT + 2 * N; i++)
         vec[i] = 0.0; // no limit on cu, cd, pc[j,1,1] and pcmax
    ...
    assert(nx==len);
      ...
      for (int i = 0; i < N * (T - 1); i++)
         vec[i] = 0.0; // no upper limit for pc //!2

      for (int k = 1; k < T; k++)
         data->loadWindScenario(id - 1, k, &vec[N * (T - 1) + (k - 1) * NW], NW);

      // no upper limit for pcmax
      for (int i = N * (T - 1) + NW * (T - 1); i < 2 * N * (T - 1) + NW * (T - 1); i++)
         vec[i] = 0.0;
#ifdef DEBUG
...
extern "C" int fixupp(void* user_data, int id, double* vec, int len) {
   ...
   if (id == 0) {
      for (int i = 0; i < N * T; i++)
         vec[i] = 1.0; //upper on v
      for (int i = N * T; i < 3 * NT + 2 * N; i++)
         vec[i] = 0.0; // no limit //!2
      for (int i = 3 * NT + 2 * N; i < 3 * NT + 2 * N + NW; i++)
         vec[i] = 1.0; //pwind upper limit
   }
   else {
      for (int i = 0; i < N * (T - 1); i++)
         vec[i] = 0.0; // no upper limit for pc//! 2
      for (int i = N * (T - 1); i < N * (T - 1) + NW * (T - 1); i++)
         vec[i] = 1.0; // upper limit for pwind//! 2
...
int PbData::get_nx(int stage) { return stage == 1 ? 3 * N * T + (2 * N + NW) : (T - 1) * (2 * N + NW); }
...
void PbData::loadWindScenario(int scenario, int tmStep, //time 0..T-1
   ...
   if (!scen_loaded) {
      ...
      char file[64];
      sprintf(file, "wind_scen%03d.dat", S);
...
void PbData::loadFromFile(const string& file) {
   ...
   string strLine;
   istringstream sstrLine;
   getline(file_op, strLine);
   sstrLine.str(strLine);
...
PbData::PbData(const string& strFile) : scen_loaded(0) {
...
void qpgenSolve(PbData& data, int printx) {
   ...
   for (int s = 1; s <= S; s++) {
      ...
      int* krowAs = new int[my + 1];
      int* jcolAs = new int[NNZA];
      double* dAs = new double[NNZA];
      int* krowBs = new int[my + 1];
      int* jcolBs = new int[NNZB];
      double* dBs = new double[NNZB];
   ...
   //root
   {
      ...
      rowind = mz0;
      colind = NNZC;
      double* iclowd = new double[mz0];
      double* icuppd = new double[mz0];

      ficlow(&data, 0, iclowd, mz0);
      fclow(&data, 0, clow, mz0);
      ficupp(&data, 0, icuppd, mz0);
      fcupp(&data, 0, cupp, mz0);
   ...
   for (int s = 1; s <= S; s++) {
      ...
      int* krowCs = new int[mz + 1];
      int* jcolCs = new int[NNZC];
      double* dCs = new double[NNZC];
      int* krowDs = new int[mz + 1];
      int* jcolDs = new int[NNZD];
      double* dDs = new double[NNZD];
      ...
      double* iclowd = new double[mz];
      double* icuppd = new double[mz];
      ficlow(&data, s, iclowd, mz);
      fclow(&data, s, &clow[mz0 + (s - 1) * mz], mz);
      ficupp(&data, s, icuppd, mz);
      fcupp(&data, s, &cupp[mz0 + (s - 1) * mz], mz);
   ...
   //root
   {
      ...
      fxlow(&data, 0, xlow, nx0);
      fxupp(&data, 0, xupp, nx0);

      double* ixlowd = new double[nx0];
      double* ixuppd = new double[nx0];
      fixlow(&data, 0, ixlowd, nx0);
      fixupp(&data, 0, ixuppd, nx0);
      for (int i = 0; i < nx0; i++) {
         if (ixlowd[i] == 0.0)
            ixlow[i] = 0;
         else
   ...
   for (int s = 1; s <= S; s++) {
      ...
      fxlow(&data, s, &xlow[nx0 + (s - 1) * nx], nx);
      fxupp(&data, s, &xupp[nx0 + (s - 1) * nx], nx);

      double* ixlowd = new double[nx];
      double* ixuppd = new double[nx];
      fixlow(&data, s, ixlowd, nx);
      fixupp(&data, s, ixuppd, nx);
   ...
   *****************************************/
    ...
    }*/
   {
      ...
      for (int i = 0; i < nnzC; i++)
         if (maxC < fabs(dC[i]))
            maxC = fabs(dC[i]);
      for (int i = 0; i < MZ; i++)
         if (maxcb < fabs(clow[i]))
            maxcb = fabs(clow[i]);
      for (int i = 0; i < NX; i++) {
         if (maxxbl < fabs(xlow[i]))
            maxxbl = fabs(xlow[i]);
         if (maxxbu < fabs(xupp[i]))
            maxxbu = fabs(xupp[i]);
         if (maxobj < fabs(c[i]))
            maxobj = fabs(c[i]);
      }
   ...
   QpGenData* prob = (QpGenData*) qp->makeData(c, krowQ, jcolQ, dQ, xlow, ixlow, xupp, ixupp, krowA, jcolA, dA, b, krowC, jcolC, dC, clow, iclow,
         cupp, icupp);
   ...
   cout << " Iterates: " << s->iter << ",    Optimal Solution:  " << objective;
   cout << ".\n";

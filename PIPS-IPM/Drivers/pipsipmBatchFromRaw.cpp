...
int main(int argc, char** argv) {
   ...
   if (argc < 6) {
      ...
      if (mype == 0)
         printf("Usage: %s [rawdump batches directory] [rawdump root name] [num batches] [num scenarios per batch] [output dir]\n", argv[0]);
      return 1;
   }
   ...
   if (nprocs < nbatch) {
      if (mype == 0)
         cout << "Number of procs has to be greater or equal to number of batches" << endl;
      return 1;
   }

   if (nbatch * (nprocs / nbatch) != nprocs) {
      if (mype == 0)
         cout << "Number of procs [" << nprocs << "] has to be multiple of the number of batches[" << nprocs / nbatch << "]" << endl;
      return 1;
   }

   if ((nprocs / nbatch) * (nscen / (nprocs / nbatch)) != nscen) {
      if (mype == 0)
         cout << "Number of scens [" << nscen << "] has to be a multiple of the number of processes per batch [" << nprocs / nbatch << "]" << endl;
      return 1;
   }
   ...
   int mynewpe;
   MPI_Comm_rank(commBatch, &mynewpe);

   stringstream ss;
   ss << datadirname << (color + 1) << "/" << datarootname;
   datarootname = ss.str();
   ...
   PIPSIPMppInterface<sFactoryAug, MehrotraStochSolver> pipsIpm(*s, commBatch);
   delete s;
   pipsIpm.go();
   ...
   for (int s = 0; s < nscen; s++) {
      ...
      if (duals.size()) {
         ...
         usleep(s * 50000);
         stringstream ss1;
         ss1 << outputdir << "/batch-" << (1 + color) << "-out_duals_scen" << (s + 1) << ".txt";
         cout << "pe[" << mype << "][" << mynewpe << "] " << " saving duals to " << ss1.str() << endl;
      ...
      if (primals.size()) {
         ...
         stringstream ss2;
         ss2 << outputdir << "/batch-" << (1 + color) << "-out_primals_scen" << (s + 1) << ".txt";
         cout << "pe[" << mype << "][" << mynewpe << "] " << " saving primals to " << ss2.str() << endl;
   ...
   if (mynewpe == 0) {
      {
         ...
         stringstream ss2;
         ss2 << outputdir << "/batch-" << (1 + color) << "-out_primal_1stStage.txt";
      ...
      }
      {
         ...
         stringstream ss2;
         ss2 << outputdir << "/batch-" << (1 + color) << "-out_dual_1stStage.txt";
         string sFile = ss2.str();

...
sLinsysLeaf* DistributedFactory::make_linear_system_leaf(DistributedQP* problem, OoqpVector* dd, OoqpVector* dq, OoqpVector* nomegaInv, OoqpVector* rhs) {
   assert(problem);
   ...
   if (!pips_options::getBoolParameter("SC_COMPUTE_BLOCKWISE")) {
      ...
      if (leaf_solver == SolverType::SOLVER_MUMPS) {
         ...
         return new sLinsysLeafSchurSlv(this, problem, dd, dq, nomegaInv, rhs);
#endif
      }
      else {
      ...
      if (leaf_solver == SolverType::SOLVER_MUMPS) {
         ...
         return new sLinsysLeaf(this, problem, dd, dq, nomegaInv, rhs);
   }
   return nullptr;
}

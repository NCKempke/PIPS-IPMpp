...
sLinsysRootAug::sLinsysRootAug(DistributedFactory* factory_, DistributedQP* prob_, OoqpVector* dd_, OoqpVector* dq_, OoqpVector* nomegaInv_,
      OoqpVector* rhs_, bool create_solvers) : sLinsysRoot(factory_, prob_, dd_, dq_, nomegaInv_, rhs_) {
...
void sLinsysRootAug::finalizeKKTdense(DistributedQP* prob, Variables*) {
   ...
   if (locmzl > 0) {
      SparseGenMatrix& G = prob->getLocalG();
      assert(zDiagLinkCons);
      SimpleVector<double>& szDiagLinkCons = dynamic_cast<SimpleVector<double>&>(*zDiagLinkCons);

...
RegularizationStrategy::RegularizationStrategy(unsigned int positive_eigenvalues_expected_,
   unsigned int negative_eigenvalues_expected_) :
   positive_eigenvalues_expected{positive_eigenvalues_expected_},
   negative_eigenvalues_expected{negative_eigenvalues_expected_}, primal_regularization_absolute_minimum{
   pips_options::getDoubleParameter("REGULARIZATION_MIN_PRIMAL")},
...
bool RegularizationStrategy::is_inertia_correct(const Inertia& inertia) const {
   ...
   return positive_eigenvalues_expected == positive_eigenvalues &&
      negative_eigenvalues_expected == negative_eigenvalues &&
      zero_eigenvalues == 0;
}
...
Regularization RegularizationStrategy::get_regularization_new_matrix(const Inertia& inertia, double barrier_parameter) {
   auto[positive_eigenvalues, negative_eigenvalues, zero_eigenvalues] = inertia;
   assert(
      positive_eigenvalues != positive_eigenvalues_expected || negative_eigenvalues != negative_eigenvalues_expected);
   assert(primal_regularization_current == -1.0 && dual_equality_regularization_current == -1.0 &&
      dual_inequality_regularization_current == -1.0);

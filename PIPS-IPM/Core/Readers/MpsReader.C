...
enum {
   kBadRowType = -1, kFreeRow, kLessRow, kGreaterRow, kEqualRow, kLessRowWithRange, kGreaterRowWithRange
};
...
void MpsReader::readColsSection(Vector<double>& c_, GenMatrix& A, GenMatrix& C, char line[], int& ierr, int& kindOfLine) {
   ...
   this->readColsSection(c.elements(), irowA, jcolA, dA, irowC, jcolC, dC, line, ierr, kindOfLine);
...
void MpsReader::readColsSection(double c[], int irowA[], int jcolA[], double dA[], int irowC[], int jcolC[], double dC[], char line[], int& ierr,
      int& kindOfLine) {
   ...
   while (DATALINE == (kindOfLine = GetLine(line))) {
      ...
      ierr = this->ParseDataLine2(line, blank, colname, row[0], &val[0], hasSecondValue, row[1], &val[1]);
      ...
      for (i = 0; i < nvals; i++) {
         ...
         switch (rowInfo[rownum].kind) {
            ...
            case kFreeRow:
               ...
               break;
            case kEqualRow:
               this->insertElt(irowA, nnzA, jcolA, dA, nea, rowRemap[rownum], colnum, val[i], ierr);
               assert(0 == ierr);
               break;
            default:
               this->insertElt(irowC, nnzC, jcolC, dC, nec, rowRemap[rownum], colnum, val[i], ierr);
               assert(0 == ierr);
...
void MpsReader::insertElt(int irow[], int len, int jcol[], double dval[], int& ne, int row, int col, double val, int& ierr) {
   if (ne >= len) {
      ierr = mpssyntaxerr;
      return;
   }
...
void MpsReader::readRHSSection(double b[], double clow[], char iclow[], double cupp[], char icupp[], char line[], int& ierr, int& kindOfLine) {
   char* seenRow = new char[totalRows];
   int i;
   for (i = 0; i < totalRows; i++)
      seenRow[i] = 0; // we haven't seen any yet
   ...
   while (DATALINE == (kindOfLine = this->GetLine(line))) {
      ierr = this->ParseDataLine2(line, blank, rhsName, row[0], &val[0], hasSecondValue, row[1], &val[1]);
      if (ierr != mpsok)
         return;
...
MpsReader::readRHSSection(Vector<double>& b_, SimpleVector<double>& clow, Vector<double>& iclow_, SimpleVector<double>& cupp, Vector<double>& icupp_,
   ...
   this->readRHSSection(db, dclow, iclow, dcupp, icupp, line, ierr, kindOfLine);
...
void MpsReader::readRangesSection(double clow[], double cupp[], char line[], int& iErr, int& kindOfLine) {
   ...
   int hasSecondValue;
   while (DATALINE == (kindOfLine = this->GetLine(line))) {
      iErr = this->ParseDataLine2(line, blank, rangeName, row[0], &val[0], hasSecondValue, row[1], &val[1]);
      assert(iErr != mpssyntaxerr);
      if (iErr != mpsok)
         return;
...
void MpsReader::defaultBounds(double xlow[], char ixlow[], double xupp[], char ixupp[]) {
   int i;
   for (i = 0; i < totalCols; i++) {
      xlow[i] = 0.0;
      ixlow[i] = 1; // Initially there is a lower bound
      xupp[i] = 0.0;
      ixupp[i] = 0; // but no upper bound
   }
}
void MpsReader::readBoundsSection(double xlow[], char ixlow[], double xupp[], char ixupp[], char line[], int& ierr, int& kindOfLine) {
   int code;
...
void MpsReader::readHessSection(int irowQ[], int jcolQ[], double dQ[], char line[], int& ierr, int& kindOfLine) {
   char colname[16];
   char name[2][16];
   char code[4];
   double val[2];
   ...
   while (DATALINE == (kindOfLine = this->GetLine(line))) {
      ...
      ierr = this->ParseDataLine2(line, code, colname, name[0], &val[0], hasSecondValue, name[1], &val[1]);
      assert(ierr != mpssyntaxerr);
      if (ierr != mpsok)
         break;
      ...
      for (i = 0; i < nvals; i++) {
         ...
         assert(rownum >= 0);
         this->insertElt(irowQ, nnzQ, jcolQ, dQ, neq, rownum, colnum, val[i], ierr);
         assert(ierr == 0);
      }
...
void MpsReader::scanRangesSection(char line[200], int& iErr, int& kindOfLine) {
   ...
   for (i = 0; i < totalRows; i++)
      seenRow[i] = 0;
   iErr = mpsok;
   while (iErr == mpsok && DATALINE == (kindOfLine = this->GetLine(line))) {
      ...
      for (i = 0; i < nvals; i++) {
         ...
         int rownum = GetIndex(rowTable, row[i]);
         if (rownum < 0) {
            fprintf(stderr, "Unrecognized row name %s at line %d.\n", row[i], iline);
            iErr = mpssyntaxerr;
            break;
         }
         if (seenRow[rownum]) {
...
void MpsReader::expectHeader(int lineType, const char expectName[], char line[], int& ierr) {
   ierr = 0;
...
void MpsReader::expectHeader2(int lineType, const char expectName[], char line[], int& ierr) {
   ierr = 0;
   if (lineType == HEADERLINE) {
      char name[16];
...
int MpsReader::acceptHeader(int lineType, const char acceptName[], char line[], int& ierr) {
   if (lineType == HEADERLINE) {
      char name[17];
      ierr = this->ParseHeaderLine(line, name);
      if (ierr != mpsok)
...
int MpsReader::acceptHeader2(int lineType, const char acceptName[], char line[], int& ierr) {
   if (lineType == HEADERLINE) {
      char name[16];
      ierr = this->ParseHeaderLine2(line, name);
...
void MpsReader::scanHessSection(char line[200], int& iErr, int& linetype) {
   ...
   if (!lastSeenRow) {
      ...
      while ((linetype = this->GetLine(line)) == DATALINE) {
         ...
         for (i = 0; i < nvals; i++) {
            int rownum = GetIndex(colTable, name[i]);
            if (rownum < 0) {
               fprintf(stderr, "Unrecognized variable name %s at line %d.\n", name[i], iline);
               iErr = mpssyntaxerr;
               break;
            }
            if (lastSeenRow[rownum] == colnum) {
               fprintf(stderr, "Element (%s, %s) specified twice.\n", name[i], colname);
               iErr = mpssyntaxerr;
               break;
            }
            if (colnum > rownum) {
               fprintf(stderr, "Error line %d: element (%s, %s)\nis not in "
...
void MpsReader::findFile(FILE*& file, char*& resolvedName, char filename[]) {
   file = nullptr;
   resolvedName = 0;
...
void MpsReader::readRowsSection(char line[200], int& iErr, int& linetype) {
   ...
   while ((linetype = this->GetLine(line)) == DATALINE) {
      ...
      if (rowType == kFreeRow) { // This may be the objective
         if (foundObjective) {
            if (foundObjective < 2) {
               fprintf(stderr, "Warning: More than one objective function was specified.\n"
...
//        (MpsRowInfo *) realloc( rowInfo, totalRows * sizeof( MpsRowInfo ) );
      ...
      if (rowTable) {
         ...
         for (i = 0; i < totalRows; i++) {
            if (Insert(rowTable, rowInfo[i].name, i) == 1) {
               fprintf(stderr, "The row name %s was used twice.\n", rowInfo[i].name);
               iErr = mpssyntaxerr;
               break;
            }
         }
   ...
   if (iErr != 0) {
      delete[] rowInfo;
      if (rowTable)
         DeleteHashTable(rowTable);
   }
}

void MpsReader::scanColsSection(char line[200], int& iErr, int& linetype) {
   ...
   if (2 * totalRows > colsGuess)
      colsGuess = 2 * totalRows;
...
void MpsReader::readQpGen(double c[], int irowQ[], int jcolQ[], double dQ[], double xlow[], char ixlow[], double xupp[], char ixupp[], int irowA[],
      int jcolA[], double dA[], double b[], int irowC[], int jcolC[], double dC[], double clow[], char iclow[], double cupp[], char icupp[],
      int& ierr) {
   char line[200];
   int kindOfLine;
   this->readColsSection(c, irowA, jcolA, dA, irowC, jcolC, dC, line, ierr, kindOfLine);
   if (ierr != mpsok)
      return;
   ...
   if (ierr != mpsok)
      return;
   this->readRHSSection(b, clow, iclow, cupp, icupp, line, ierr, kindOfLine);

   // RANGES (optional)
   if (this->acceptHeader2(kindOfLine, "RANGES", line, ierr)) {
      this->readRangesSection(clow, cupp, line, ierr, kindOfLine);
   }
   if (ierr != mpsok)
      return;
   ...
   // BOUNDS (optional)
   if (this->acceptHeader2(kindOfLine, "BOUNDS", line, ierr)) {
      this->readBoundsSection(xlow, ixlow, xupp, ixupp, line, ierr, kindOfLine);
   }
   else {
      this->defaultBounds(xlow, ixlow, xupp, ixupp);
   }
...
int MpsReader::GetLine_old(char* line) {
   ...
   do {
      ...
      while (i < length && EOF != (c = getc(file))) {
         ...
         if (!isprint(c))
            c = ' '; // Eliminate non-printing characters
         line[i] = c;
      ...
      if (!terminated) {
         ...
         if (i == 0) {
            ...
            if (line[0] == '*') {
               ...
               if (extracrud) {
                  fprintf(stderr, "Extra characters beyond column 61 in line %d.\n", iline);
                  return mpssyntaxerr;
               }
      ...
      if (!terminated) {
         ...
         if (i == 0) {
            ...
            if (line[0] == '*') {
               ...
               if (extracrud) {
                  fprintf(stderr, "Line %d has exceeded the maximum permissible characters.\n", iline);
                  return mpssyntaxerr;
               }
            } // end else this is not a comment line
...
int MpsReader::ParseHeaderLine(char line[], char entry[]) {
   ...
   if (extra_crud) {
      fprintf(stderr, "Extra characters outside prescribed fields at line %d.\n", iline);
      return mpssyntaxerr;
   }
...
int MpsReader::ParseBoundsLine(char line[], int& code, char name1[], char name2[], double* val) {
   int extra_crud = 0;
   char codeStr[4];
   char valstr[16];
   int ierr;
...
int MpsReader::ParseBoundsLine2(char line[], int& code, char name1[], char name2[], double* val) {
   int i = 0;
   char* token;
...
int MpsReader::ParseDataLine(char line[], char code[], char name1[], char name2[], double* val1, int& hasSecondValue, char name3[], double* val2) {
   ...
   *val2 = 0.0;
   ...
   extra_crud = extra_crud || ' ' != line[36] || ' ' != line[37] || ' ' != line[38];
...
int MpsReader::ParseDataLine2(char line[], char[], char name1[], char name2[], double* val1, int& hasSecondValue, char name3[], double* val2) {
   int i = 0;
   char* token;
...
char* MpsReader::defaultOutputFilename(int& iErr) {
   if (infilename == nullptr) {
      ...
      fprintf(stderr, "Apparently no input file, so can't construct an output file.\n");
      iErr = mpsioerr;
      return 0;
   }
...
void MpsReader::printSolution(double x[], int nx, double xlow[], char ixlow[], double xupp[], char ixupp[], double gamma[], double phi[], double y[],
      int mA, double s[], int mC, double clow[], char iclow[], double cupp[], char icupp[], double lambda[], double pi[], double z[],
      double objective, int& iErr) {
   /** output file */
   FILE* outfile;
...

   {
      ...
      if (outfile == nullptr) {
         fprintf(stderr, "Unable to open output file '%s'; solution not printed.\n", outfilename);
         iErr = mpsfileopenerr;
         return;
      }
   ...
   for (int i = 0; i < nx; i++) {
      if (ixlow[i] == 0.0 && ixupp[i] == 0.0) {
         ...
         fprintf(outfile, " %5d %8s %15.8e   %15.8e  %15.8e  %15.8e\n", i, colInfo[i].name, x[i], xlow[i], xupp[i], gamma[i] - phi[i]);
      }
   }
   ...
   if (mA > 0) {
      ...
      for (int i = 0; i < mA; i++) {
         ...
         // file)
         while (i_equalities < totalRows && rowInfo[i_equalities].kind != kEqualRow)
            i_equalities++;
   ...
   if (mC > 0) {
      ...
      for (int i = 0; i < mC; i++) {
         ...
         while (i_inequalities < totalRows && (rowInfo[i_inequalities].kind == kEqualRow || rowInfo[i_inequalities].kind == kFreeRow))
            i_inequalities++;
         if (iclow[i] == 1.0 && icupp[i] == 0.0) {
            ...
            fprintf(outfile, " %5d %8s %15.8e   %15.8e  %15.8e  %15.8e\n", i, rowInfo[i_inequalities].name, s[i], clow[i], cupp[i], z[i]);
         }
         i_inequalities++;
      }
   }

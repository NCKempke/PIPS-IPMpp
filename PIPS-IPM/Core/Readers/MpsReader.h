 ...
 *
 *  The problem to be read is in QP format:
 *  
 *  <pre>
...
protected:
   ...
   virtual int ParseDataLine2(char line[], char[], char name1[], char name2[], double* val1, int& hasSecondValue, char name3[], double* val2);
   ...
   virtual void readColsSection(double c[], int irowA[], int jcolA[], double dA[], int irowC[], int jcolC[], double dC[], char line[62], int& iErr,
         int& return_getline);
   virtual void
   readRHSSection(Vector<double>& b, SimpleVector<double>& clow, Vector<double>& iclow, SimpleVector<double>& cupp, Vector<double>& icupp, char line[], int& ierr,
         int& kindOfLine);
   ...
   virtual void readBoundsSection(Vector<double>& xlow, Vector<double>& ixlow, Vector<double>& xupp, Vector<double>& ixupp, char line[], int& ierr, int& kindOfLine);
...
public:
   ...
   static void findFile(FILE*& file, char*& resolvedName, char filename[]);
   /**
    ...
    */
   virtual void readQpBound(Vector<double>& c, SymMatrix& Q, Vector<double>& xlow, Vector<double>& ixlow, Vector<double>& xupp, Vector<double>& ixupp, int& ierr);
   /**
    * Reads the various components of a QP in the "general" formulation
    * into their respective matrices and vectors, stored as objects
    * from OOQP's linear algebra classes. See the class comments for
   ...
   virtual void
   readQpGen(Vector<double>& c, SymMatrix& Q, Vector<double>& xlow, Vector<double>& ixlow, Vector<double>& xupp, Vector<double>& ixupp, GenMatrix& A, Vector<double>& b,
         GenMatrix& C, Vector<double>& clow, Vector<double>& iclow, Vector<double>& cupp, Vector<double>& icupp, int& ierr);
   ...
   virtual void
   readQpGen(double c[], int irowQ[], int jcolQ[], double dQ[], double xlow[], char ixlow[], double xupp[], char ixupp[], int irowA[], int jcolA[],
         double dA[], double b[], int irowC[], int jcolC[], double dC[], double clow[], char iclow[], double cupp[], char icupp[], int& ierr);
   /**
   ...
   /** print the solution contained in the individual arrays of the
       ...
       structure */
   void printSolution(double x[], int nx, double xlow[], char ixlow[], double xupp[], char ixupp[], double gamma[], double phi[], double y[], int my,
         double s[], int mz, double clow[], char iclow[], double cupp[], char icupp[], double lambda[], double pi[], double z[],
         double objective_value, int& iErr);
   char* defaultOutputFilename(int& iErr);

};

enum {
   mpsok = 0, mpsunknownerr = -1, // Someone didn't set the error code properly.

...
template<typename T>
void StochVectorBase<T>::writefToStream(std::ostream& out, const char format[]) const {
   if (first)
 ...
 */
template<typename T>
T StochVectorBase<T>::shiftedDotProductWith(T alpha, const OoqpVectorBase<T>& mystep_, const OoqpVectorBase<T>& yvec_, T beta,
...
template<typename T>
void StochVectorBase<T>::axdzpy(T alpha, const OoqpVectorBase<T>& x_, const OoqpVectorBase<T>& z_, const OoqpVectorBase<T>& select_) {
   const StochVectorBase<T>& select = dynamic_cast<const StochVectorBase<T>&>(select_);
   const StochVectorBase<T>& x = dynamic_cast<const StochVectorBase<T>&>(x_);
   const StochVectorBase<T>& z = dynamic_cast<const StochVectorBase<T>&>(z_);
...
n_links_in_root
   ...
   for (unsigned int i = 0; i < n_new_children; ++i) {
      ...
      StochVectorBase<T>* vec = (child_comms[i] == MPI_COMM_NULL) ? nullptr : new StochVectorBase<T>(new SimpleVector<T>(0), vecl_child,
            child_comms[i]);
...
template<typename T>
void StochVectorBase<T>::appendHierarchicalToThis(SimpleVector<T>* new_vec, SimpleVector<T>* new_vecl, std::vector<StochVectorBase<T>*>& new_children,
   ...
   for (size_t i = 0; i < children.size(); ++i) {
      ...
      if (sub_root) {
         if (child.isKindOf(kStochDummy))
            ...
            assert(!child.last);
            dynamic_cast<StochVectorBase<T>&>(*child.first).appendHierarchicalToThis(new_vec, new_vecl, new_children, *sub_root,
...
template<typename T>
void StochDummyVectorBase<T>::appendHierarchicalToThis(SimpleVector<T>*, SimpleVector<T>* new_vecl, std::vector<StochVectorBase<T>*>& new_children,
...
template<typename T>
void StochVectorBase<T>::pushSmallComplementarityPairs(OoqpVectorBase<T>& other_vec_in, const OoqpVectorBase<T>& select_in, double tol_this,
...
template
class StochVectorBase<int>;

template
class StochVectorBase<double>;

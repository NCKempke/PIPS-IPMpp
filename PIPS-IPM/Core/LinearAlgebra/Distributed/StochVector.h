...
public:
    ...
    */
   T shiftedDotProductWith(T alpha, const OoqpVectorBase<T>& mystep, const OoqpVectorBase<T>& yvec, T beta,
         const OoqpVectorBase<T>& ystep) const override;
   ...
   virtual void appendHierarchicalToThis(SimpleVector<T>* new_vec, SimpleVector<T>* new_vecl, std::vector<StochVectorBase<T>*>& new_children,
...
private:
   ...
   void pushSmallComplementarityPairs(OoqpVectorBase<T>& other_vec_in, const OoqpVectorBase<T>& select_in, double tol_this, double tol_other,
...
public:
   ...
   T find_blocking(const OoqpVectorBase<T>&, const OoqpVectorBase<T>&, const OoqpVectorBase<T>&, T maxStep, T*, T*, T*, T*,
         int&) const override { return maxStep; }

   void find_blocking_pd(const OoqpVectorBase<T>&, const OoqpVectorBase<T>&, const OoqpVectorBase<T>&, T&, T&, T&, T&, T&, T&, T&, T&, T&, T&, bool&,
         bool&) const override {};
   ...
   StochVectorBase<T>* raiseBorder(int, bool, bool) override {
      assert(0 && "This should never be attempted");
      return nullptr;
   };

   void appendHierarchicalToThis(SimpleVector<T>* new_vec, SimpleVector<T>* new_vecl, std::vector<StochVectorBase<T>*>& new_children,
         const sTree& tree_hier, const DistributedQP& data_hier, VectorType type, bool empty_vec) override;

   OoqpVectorBase<T>* getLinkingVecNotHierarchicalTop() const override {

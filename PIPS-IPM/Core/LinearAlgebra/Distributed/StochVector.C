...
template<typename T>
T StochVectorBase<T>::find_blocking(const OoqpVectorBase<T> & wstep_vec,
   ...
   int& first_or_second) const
{
   ...
   if( w.last )
   {
      ...
      step = w.last->find_blocking(*wstep.last, *u.last, *ustep.last, step,
   ...
   if( w.first )
   {
      ...
      step = w.first->find_blocking(*wstep.first, *u.first, *ustep.first, step,
         w_elt, wstep_elt, u_elt, ustep_elt,
         first_or_second);
   }
   ...
   {
      step = w.children[it]->find_blocking(*wstep.children[it], *u.children[it], *ustep.children[it], step,
...
template<typename T>
void StochVectorBase<T>::find_blocking_pd(const OoqpVectorBase<T> & wstep_vec,
   const OoqpVectorBase<T> & u_vec,
   const OoqpVectorBase<T> & ustep_vec,
   ...
   bool& primalBlocking, bool& dualBlocking) const
{
   ...
   if( w.last )
   {
      ...
      w.last->find_blocking_pd(*wstep.last, *u.last, *ustep.last, maxStepPri, maxStepDual,
   ...
   if( w.first )
   {
      ...
      w.first->find_blocking_pd(*wstep.first, *u.first, *ustep.first, maxStepPri, maxStepDual,
         w_elt_p, wstep_elt_p, u_elt_p, ustep_elt_p,
         w_elt_d, wstep_elt_d, u_elt_d, ustep_elt_d,
         primalBlocking, dualBlocking);
   }
   ...
   for(int it=0; it<nChildren; it++) {
      w.children[it]->find_blocking_pd(*wstep.children[it],
         *u.children[it],
         *ustep.children[it],
...
)
{
   ...
   for( unsigned int i = 0; i < n_new_children; ++i )
   {
      ...
      StochVectorBase<T>* vec = (child_comms[i] == MPI_COMM_NULL) ? nullptr :
         new StochVectorBase<T>( new SimpleVector<T>(0), vecl_child, child_comms[i] );
...
template<typename T>
void StochVectorBase<T>::appendHierarchicalToThis( SimpleVector<T>* new_vec, SimpleVector<T>* new_vecl,
...
template<typename T>
void StochDummyVectorBase<T>::appendHierarchicalToThis( SimpleVector<T>*, SimpleVector<T>* new_vecl,

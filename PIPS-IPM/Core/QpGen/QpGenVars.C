...
QpGenVars::QpGenVars(OoqpVector* x_in, OoqpVector* s_in, OoqpVector* y_in, OoqpVector* z_in, OoqpVector* v_in, OoqpVector* gamma_in, OoqpVector* w_in,
...
QpGenVars::QpGenVars(LinearAlgebraPackage* la, long long nx_, long long my_, long long mz_, OoqpVector* ixlow_in, OoqpVector* ixupp_in,
      OoqpVector* iclow_in, OoqpVector* icupp_in) {
   SpReferTo(ixlow, ixlow_in);
   SpReferTo(ixupp, ixupp_in);
   SpReferTo(iclow, iclow_in);
...
double
QpGenVars::findBlocking(const Variables* step, double& primalValue, double& primalStep, double& dualValue, double& dualStep, int& firstOrSecond) {
   double alpha = 1.0;
   firstOrSecond = 0;
...
void
QpGenVars::findBlocking_pd(const Variables* step, double& primalValue, double& primalStep, double& dualValue, double& dualStep, double& primalValue_d,
...
void QpGenVars::unscaleSolution(QP* data) {
...
void QpGenVars::unscaleBounds(QP* data) {
...
void QpGenVars::printSolution(MpsReader* reader, QP* data, int& iErr) {
   ...
   double objective;
   {
      ...
      data->getg(*temp);
      data->hessian_multiplication(1.0, *temp, 0.5, *x);
...
void QpGenVars::setNotIndicatedBoundsTo(Problem& data, double value) {
   ...
   QP& qpdata = dynamic_cast<QP&>(data);

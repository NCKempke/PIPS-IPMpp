...
void LinearSystem::solve(Problem* problem, Variables* variables, Residuals* residuals, Variables* step) {
...

   {
      ...
      if (mclow > 0) {
         ztemp = step->slack_lower_bound_gap_dual;
      }
      else {
         ztemp = step->slack_upper_bound_gap_dual;
      }

      solveXYZS(*step->primals, *step->equality_duals, *step->inequality_duals, *step->slacks, *ztemp, problem);
   }

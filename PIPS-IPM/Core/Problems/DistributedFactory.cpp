...
Problem* DistributedFactory::make_problem() const {
   ...
   return new DistributedProblem(tree, std::move(c), std::move(Q), std::move(xlow), std::move(ixlow), std::move(xupp),
...
std::unique_ptr<DistributedRootLinearSystem> DistributedFactory::make_linear_system_root(DistributedProblem* problem) {
   assert(problem);
...
DistributedFactory::make_root_hierarchical_linear_system(DistributedProblem* problem) {

...
void Ma57Solver::matrixChanged()
{
   ...
   do
   {
      FNAME(ma57bd)( &n, &nnz, mat_storage->M, fact.data(), &lfact, ifact.data(),
            &lifact, &lkeep, keep.data(), iworkn.data(), icntl.data(), cntl.data(), info.data(), rinfo.data() );
      ...
      else
      {
         if( thresholdPivoting() < threshold_pivoting_max )
         {
            ...
            if( gOoqpPrintLevel >= ooqp_print_level_warnings )
               std::cout << "WARNING MA57 " << name << ": Setting ThresholdPivoting parameter to " << thresholdPivoting() << " for future factorizations\n";
         }
...
void Ma57Solver::solve(int solveType, OoqpVector& rhs_in)
{
   ...
   else
   {
      ...
      FNAME(ma57cd)(&job, &n, fact.data(), &lfact, ifact.data(), &lifact, &one_rhs, drhs, &n, dworkn.data(), &n, iworkn.data(), icntl.data(), info.data());
   }
}
...
bool Ma57Solver::checkErrorsAndReact()
{
   ...
   switch ( error_flag )
   {
      ...
      {
         if( gOoqpPrintLevel >= ooqp_print_level_warnings )
            std::cout<< "WARNING M57 " << name << ": detected " << info[3] << " duplicate entries in user supplied matrix detected - summing them up\n";
      } break;
      case 3:
      {
         if( gOoqpPrintLevel >= ooqp_print_level_warnings )
      ...
      {
         if( gOoqpPrintLevel >= ooqp_print_level_warnings )
            std::cout << "WARNING MA57 " << name << ": pivots have different sign when factorizing supposedly definite matrix; " << info[25] << " sign changes detected\n";
      }; break;
      case 8:
      {
         if( gOoqpPrintLevel >= ooqp_print_level_warnings )
